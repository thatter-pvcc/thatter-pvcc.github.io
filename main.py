...
HUNGER = 30 # Hunger mechanic was never implemented, it is left here for functionality.
...
while True:
    ...
    running = True
    while running:
        dt = clock.tick(FPS_LIMIT) / 1000 # Derived from an example file on https://www.pygame.org/docs/.
        MOVE_TIMER += dt
        ...
        if MOVE_TIMER >= MOVE_INTERVAL: # Derived from an example file on https://www.pygame.org/docs/.
            MOVE_TIMER = 0  # Reset the timer
            if not snake.move() or snake.check_collision():
                running = False
